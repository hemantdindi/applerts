...
public class MailAlert {
    ...
    public boolean sendMail(AppPOJO ai, String smtphost, int smtpport, String smtpsender, String alertCC, String recepient){
        ...
        try {
            log.info("fetching details : "+ai.getId());
            ...
            log.info("fetched details : "+ai.getId());
            if(recepient.equals("noemail"))
                email.addTo(alertCC);
            else {
                email.addTo(recepient.trim());
                email.addCc(alertCC);
            }
            email.send();
            isDelivered=true;
